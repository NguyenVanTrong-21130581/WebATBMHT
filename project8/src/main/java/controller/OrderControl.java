...
public class OrderControl extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            String dataToSign = name + sdt + address + content + totalMoney + key;
            ...
            String signedDataOrder = signOrder(hashDataOrder, pvKey);

            if (verifySignature(hashDataOrder, signedDataOrder, plKey)) {
                dao.saveOrder(accountID, name, sdt, email, address, content, totalMoney, signedDataOrder);
                ...
                request.setAttribute("error", "Xác minh chữ ký thất bại");
                request.getRequestDispatcher("datHang.jsp").forward(request, response);
            }
        } catch (Exception e) {
